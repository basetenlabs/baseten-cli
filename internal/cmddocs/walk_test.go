...
func Test_Walk_RecursesIntoChildren(t *testing.T) {
	...
	got := WalkCommand([]string{"baseten"}, tree)
	if got.IsLeaf {
...
func Test_Walk_OutputAndExamples(t *testing.T) {
	...
	if got.JSONOutputType != "main.pingResult" {
		t.Fatalf("JSONOutputType = %q, want %q", got.JSONOutputType, "main.pingResult")
	}
}
...
func Test_Walk_OutputNilForParent(t *testing.T) {
	...
	got := WalkCommand([]string{"baseten"}, parent)
...
func Test_Walk_TopLevelMetadata(t *testing.T) {
	...
	s := Walk("v9.9.9", "2026-01-01T00:00:00Z", root)
