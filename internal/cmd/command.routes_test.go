...
func Test_Routes_List_Pagination(t *testing.T) {
	...
	defer server.Close()
	routes, err := internalcmd.ListRoutesForTest(t.Context(), &managementapi.Client{HTTPClient: server.Client(), BaseURL: server.URL, Headers: http.Header{"Authorization": []string{"Bearer mock"}}}, managementapi.GetV1RoutesParams{TeamId: ptrTeam("team-a")})
...
func Test_Routes_List_RejectsInvalidPages(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer server.Close()
			_, err := internalcmd.ListRoutesForTest(t.Context(), &managementapi.Client{HTTPClient: server.Client(), BaseURL: server.URL}, managementapi.GetV1RoutesParams{TeamId: ptrTeam("team-a")})
			require.Error(t, err)
		})
	}
}
...
func Test_Routes_Key_Create(t *testing.T) {
	...
	defer server.Close()
	key, err := internalcmd.CreateRoutesKeyForTest(t.Context(), &managementapi.Client{BaseURL: server.URL, HTTPClient: &auth.Transport{Session: session}}, "test-login", "test-key", "team-a")
	require.NoError(t, err)
...
func Test_Routes_Key_RejectionPreservesTypeAndRedacts(t *testing.T) {
	...
	defer server.Close()
	_, err = internalcmd.CreateRoutesKeyForTest(t.Context(), &managementapi.Client{BaseURL: server.URL, HTTPClient: &auth.Transport{Session: session}}, "test-login", "test-key", "team-a")
...
func ptrTeam(team string) *string { return &team }
