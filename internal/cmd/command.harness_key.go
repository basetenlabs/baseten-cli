...
func init() {
	Register("harness key list", commandHarnessKeyList)
	Register("harness key revoke", commandHarnessKeyRevoke)
}

// listHarnessKeys returns the routes API keys the caller created, across all
// their machines and teams.
func listHarnessKeys(ctx context.Context, api *managementapi.Client) (*managementapi.APIKeys, error) {
...
func commandHarnessKeyList(ctx *CommandContext, _ *cmd.HarnessKeyListFlags) error {
	cl, err := ctx.NewManagementClient()
	if err != nil {
		return err
	}
	keys, err := listHarnessKeys(ctx, cl.API())
	if err != nil {
		return err
	}
	if ctx.JSON {
...
func commandHarnessKeyRevoke(ctx *CommandContext, flags *cmd.HarnessKeyRevokeFlags) error {
	cl, err := ctx.NewManagementClient()
	if err != nil {
		return err
	}
	...
	if !flags.All {
		...
		if len(targets) == 0 {
			return fmt.Errorf("no routes API key with prefix %q; run 'baseten harness key list'", flags.Prefix)
		}
	}
	result := cmd.HarnessKeyRevokeResult{Revoked: []string{}, Failed: []string{}}
	if len(targets) == 0 {
		if ctx.JSON {
