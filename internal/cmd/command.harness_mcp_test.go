...
func Test_Harness_Setup_MCPServersWritten(t *testing.T) {
	...
	h.Require.NoError(h.Execute("harness", "setup", "--harness", "claude-code", "--model", "acme/primary", "--team", "team-a", "--key-name", "mcp", "--config", path, "--yes", "--output", "json"))
	...
	info, err := os.Stat(filepath.Join(dir, ".claude.json.baseten-harness.json"))
	h.Require.NoError(err)
	h.Require.Zero(info.Mode().Perm() & 0077)
...
func Test_Harness_Setup_MCPServers404Proceeds(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", dir)
	path := filepath.Join(t.TempDir(), "settings.json")
	h.Require.NoError(h.Execute("harness", "setup", "--harness", "claude-code", "--model", "acme/primary", "--team", "team-a", "--key-name", "absent", "--config", path, "--yes"))
