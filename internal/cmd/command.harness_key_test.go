...
func Test_Harness_Key_List(t *testing.T) {
	h, api := harnessAPI(t, "")
	h.Require.NoError(h.Execute("harness", "key", "list"))
...
func Test_Harness_Key_Revoke(t *testing.T) {
	h, api := harnessAPI(t, "")
	...
	h.Require.Error(h.Execute("harness", "key", "revoke"))
	h.Require.Equal(int(public.ExitUsage), h.ExitCode)
	h.Require.Error(h.Execute("harness", "key", "revoke", "--prefix", "secret-team-a"))
	h.Require.Contains(h.Stderr.String(), "pass --yes")
	h.Require.Error(h.Execute("harness", "key", "revoke", "--prefix", "unknown", "--yes"))
	...
	h.Require.NoError(h.Execute("harness", "key", "revoke", "--prefix", "secret-team-a", "--yes"))
...
func Test_Harness_Key_RevokeAllReportsPartialFailure(t *testing.T) {
	h, api := harnessAPI(t, "")
	api.SetRoute("DELETE", "/v1/api_keys/secret-team-a", 500, map[string]string{"message": "unavailable"})
	api.SetRoute("DELETE", "/v1/api_keys/secret-team-b", 200, map[string]string{"prefix": "secret-team-b"})
	h.Require.Error(h.Execute("harness", "key", "revoke", "--all", "--yes", "--output", "json"))
	var result public.HarnessKeyRevokeResult
	h.Require.NoError(json.Unmarshal(h.Stdout.Bytes(), &result))
	h.Require.Equal(public.HarnessKeyRevokeResult{Revoked: []string{"secret-team-b"}, Failed: []string{"secret-team-a"}}, result)
}
